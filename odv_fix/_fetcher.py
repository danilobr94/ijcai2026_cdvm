"""FIX OpenDataVal DataFetcher for GPU"""
import json
import warnings
...
class DataFetcher:
    ...
    def _presplit_data(self, x_train, x_valid, x_test, y_train, y_valid, y_test):
        ...
        self.x_train, self.x_valid, self.x_test = x_train.to(self.device), x_valid, x_test
    ...
    def to_device(self, device):
        ...
        def maybe_to_device(data):
            return data.clone().detach().to(device) if isinstance(data, torch.Tensor) else torch.tensor(data, dtype=torch.float).to(device)

        self.x_train = maybe_to_device(self.x_train)
        self.x_valid = maybe_to_device(self.x_valid)
        self.x_test = maybe_to_device(self.x_test)

        self.y_train = maybe_to_device(self.y_train)
        self.y_valid = maybe_to_device(self.y_valid)
        self.y_test = maybe_to_device(self.y_test)
